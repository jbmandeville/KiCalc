...
#define FUNC_TEMP  qInfo()  << Q_FUNC_INFO
...
struct mapTypes
{
    QString mapName;         // condition-type name (e.g., "P" or "BP")
    QString fileName;        // file-name including map and condition (e.g., "P-1" or "BP-A")
    fileCategory category;   // e.g. category_pValue
    int nValues;             // usually 2
    dVector values;          // mean, error, ... [dim=nValuesPerMap]
...
{   // dimension this to [nConditions]; this holds condition values for a single overlay
    ...
    QString conditionName;  // e.g., "1" or "a-b"
    int DOF;
    QVector<mapTypes> mapType;
};
...
namespace utilIO
{
    ...
    inline bool voxelInBounds(iPoint3D voxel, iPoint3D dim) {return
                voxel.x>=0    && voxel.y>=0    && voxel.z>=0 &&
                voxel.x<dim.x && voxel.y<dim.y && voxel.z<dim.z;}
    inline bool voxelInBounds(fVoxel voxel, iPoint3D dim) {return
                voxel.x>=0    && voxel.y>=0    && voxel.z>=0 &&
                voxel.x<dim.x && voxel.y<dim.y && voxel.z<dim.z;}

    iPoint4D getImageDimensions(QString fileName);
...
#define NMAT 3
    ...
    basisFunction createBasisShape(iPoint3D grid);
    void defineBasis1DBSpline3(iPoint3D grid, dMatrix &basis1d);
    double basisValue1dBSpline3(double xAbs, double xRel);
