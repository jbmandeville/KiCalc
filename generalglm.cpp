...
//    FUNC_ENTER;
    ...
    _conditionList = conditionString.split(QRegExp("[\\s]"), QString::SkipEmptyParts);
    decodeConditions(false);  // this excludes bad conditions in the string and reset the string to valid conditions
    decodeConditions(true);
//    FUNC_EXIT;
}
...
//    FUNC_ENTER << timeVector;
    ...
    for ( int jt=0; jt<nTime; jt++)
    {
        ...
        if ( linkWidthToTime ) smoothingHalfWidth = time0 * smoothingScaleNonzero;    // link smoothing width to time, e.g., smoothingScale=0.5
